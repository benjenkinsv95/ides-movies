...
// import our signUp and signIn axios calls
...
class SignUp extends Component {
  ...
  onSignUp = event => {
    ...
    signUp(this.state)
      // if we could sign up successfully, then try to signIn wuth the same email and password
      .then(() => signIn(this.state))
      // if we signed in successfully, set the user to the user we got back
      // in the response's data
      .then(res => setUser(res.data.user))
      // show a successful sign up message.
      .then(() => msgAlert({
        heading: 'Sign Up Success',
        message: messages.signUpSuccess,
        // the message will be green
        variant: 'success'
      }))
      ...
      .catch(error => {
        ...
        // then show an error message to our user
        msgAlert({
  ...
  render () {
    ...
    return (
      // add a bootstrap row and different column sizes, so the sign up form is
      // responsive on different sized pages.
      <div className="row">
        {/*
          * mt-5 sets the `top` `margin` to be large (5)
          * mx-auto sets the x margins (left & right) automatically. this centers
          *         the div horizontally, because the left and right margin are the same.
          */}
        <div className="col-sm-10 col-md-8 mx-auto mt-5">
          <h3>Sign Up</h3>
          {/* When the sign up form is submitted, run the `onSignUp` function */}
          <Form onSubmit={this.onSignUp}>
            {/* A form group, groups a form label, control (input), and optional help text */}
            <Form.Group controlId="email">
              <Form.Label>Email address</Form.Label>
              {/* This is similar to an input in the `react-crud` lesson. But
                  since we are using react bootstrap, we use a Form.Control instead */}
              <Form.Control
                required
                type="email"
                name="email"
